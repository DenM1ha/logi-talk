...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.send_button = CTkButton(self, text="➡", width=50, height=40, command=self.send_msg)
        ...
        self.add_msg("Демонстрація відображення зображення:",
              CTkImage(Image.open("ProjectsM3/img.png"), size=(300, 200)))
    ...
    def toggle_menu(self):
        if self.is_menu_shown:
            ...
            if self.avatar_image:              self.avatar_preview.configure(image=self.avatar_image, text="")
    ...
    def show_menu(self):
        ...
        if not self.menu_frame.winfo_width() >= 200 and self.is_menu_shown:
            ...
            # Видалення віджетів з меню перед закриттям
    ...
    def add_msg(self, message, img=None, avatar=None):
        ...
        if not img:
            CTkLabel(msg_frame, text=message,
            		wraplength=wrap_size, text_color='white',
